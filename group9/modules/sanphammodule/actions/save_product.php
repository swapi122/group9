...
if (!defined("EXPONENT")) exit("");
	...
	if (exponent_permissions_check("manage",$loc)) {	
		...
		if ($_FILES['bigimage']['name'] != '') {
			$file = file::update("bigimage",$directory,null,time()."_".$_FILES['bigimage']['name']);
			if ($file == null) {
				switch($_FILES["bigimage"]["error"]) {
					case UPLOAD_ERR_INI_SIZE:
					case UPLOAD_ERR_FORM_SIZE:
						$post['_formError'] = "The file you attempted to upload is too large.  Contact your system administrator if this is a problem.";
						break;
		...
		if ($file != null) {
			...
			{
				$myfile = $db->selectObject('file','id='.$listing->bigimage_file_id);
				file::delete($myfile);
			}
			$listing->bigimage_file_id = $db->insertObject($file, 'file');
		} else {
			if (!isset($listing->id)) {
				$listing->bigimage_file_id = 0;
			}
		}
		// cập nhật database
		if (isset($listing->id)) {
