...
<b>{$link->name}</b><br/>
			<a title="{$link->description}" href="http://edit.yahoo.com/config/send_webmesg?.target={$link->url}&amp;.src=pg"><img width="125" height="25" border="0" src="http://opi.yahoo.com/online?u={$link->url}&amp;m=g&amp;t=2&amp;l=us" alt="{$link->name} " title="{$link->name}" {if $link->opennew == 1} target="_blank"{/if} ></a>
	<br/>
	{foreachelse}
