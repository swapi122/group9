...
<div class="itemtitle news_itemtitle">Tin tức mới về {$tag_name}</div>
{foreach from=$tag item=newsitem}
	...
	<div>
		<li class="news_itemtitle">
			...
			Posted: {$newsitem->posted|format_date:"%B %d, %Y"}<br />[<a href="{link action=view id=$newsitem->id}">đọc thêm</a>]
		</li>
		{permissions level=$smarty.const.UILEVEL_PERMISSIONS}
