...
class doitacmodule {
	function name() { return 'Đối tác module'; }
	function description() { return 'Tạo danh sách các đối tác'; }
	function author() { return 'Phong'; }
	...
	function show($view,$loc = null, $title = '') {
		...
		$template = new template('doitacmodule',$view,$loc);
		
		if (!defined('SYS_SORTING')) require_once(BASE.'subsystems/sorting.php');
		if (!defined('SYS_FILES')) require_once(BASE.'subsystems/files.php');
		
		$directory = 'files/doitacmodule/' . $loc->src;
	...
	function spiderContent($item = null) {
		...
		$search->ref_module = 'doitacmodule';
		$search->ref_type = 'doitac';
		
		if ($item) {
			$db->delete('search',"ref_module='doitacmodule' AND ref_type='doitac' AND original_id=" . $item->id);
			$search->original_id = $item->id;
			$search->title = ' ' . $item->name . ' ';
			$search->view_link = 'index.php?module=doitacmodule&action=view_listing&id='.$item->id;
			$search->body = ' ' . exponent_search_removeHTML($item->body) . ' ';
			$search->location_data = $item->location_data;
			$db->insertObject($search,'search');
		} else {
			$db->delete('search',"ref_module='doitacmodule' AND ref_type='doitac'");
			foreach ($db->selectObjects('doitac') as $item) {
				$search->original_id = $item->id;
				$search->title = ' ' . $item->name . ' ';
				$search->view_link = 'index.php?module=doitacmodule&action=view_listing&id='.$item->id;
				$search->body = ' ' . exponent_search_removeHTML($item->body) . ' ';
